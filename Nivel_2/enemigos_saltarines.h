...
class Enemigos_saltarines: public QObject
{
    Q_OBJECT
    float px;
    float py;
    ...
    float rr;
public:
    Enemigos_saltarines(float posx_, float posy_, float velx_, float vely_,float masa_, float radio_, float k_, float e_, float rr_);
    ~Enemigos_saltarines();
    void Actualizar();
    float getPx() const;
    float getPy() const;
